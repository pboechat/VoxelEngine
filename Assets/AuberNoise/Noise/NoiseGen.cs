namespace Aubergine.Noise {
	...
	public enum NoiseQuality {
		Fast = 0, Standard = 1, Best = 2
	}
	
	public class NoiseGen {
		//Constants used by current version
		...
		//Generates a gradient-coherent-noise value
		public double GradientCoherentNoise3D(double x, double y, double z, int seed,
		...
		public double GradientNoise3D(double fx, double fy, double fz, int ix,
			...
			//Setup us another vector equal to the distance between two vectors passed to this function.
			double xvPoint = (fx - (double)ix);
			double yvPoint = (fy - (double)iy);
			double zvPoint = (fz - (double)iz);
			//Now compute the dot product of the gradient vector
			return ((xvGradient * xvPoint) + (yvGradient * yvPoint) + (zvGradient * zvPoint)) * 2.12;
		}

		//Generates a value noise
		public int IntValueNoise3D(int x, int y, int z, int seed  = 0) {
			//All constants are primes and must remain prime for this noise to function correctly.
			int n = (
		...
		//Value Noise
		public double ValueNoise3D(int x, int y, int z, int seed = 0) {
